# PROMPT DE CONTINUACAO - ULISSES (Tech Lead)
...
- **Infra:** PM2 (cluster mode) + Nginx + Let's Encrypt SSL
...
4. **SEMPRE faca pull antes de comecar** para evitar conflitos com o Weverton
...
# Deploy completo (com backup automatico)
ssh root@76.13.231.9 "cd /var/www/nucleoia && bash scripts/deploy.sh"

# Rollback
ssh root@76.13.231.9 "cd /var/www/nucleoia && bash scripts/rollback.sh"
```
...
- 2.498 prompts importados (categoria Criativo) com imagens locais WebP
- Sistema de cursos/aulas com upload
- Webhooks Lastlink + Greenn
- i18n (pt-BR, en, es)
- Monitoramento de IP
- Git + GitHub configurado (main + develop)
- Documentacao completa em /docs/
- Scripts de backup/deploy/rollback
